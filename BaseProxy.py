...
class ProxyHandle(BaseHTTPRequestHandler):
    ...
    def __init__(self,request,client_addr,server):
        ...
        self.is_request_blocked = Event()
        self.is_response_blocked = Event()
    ...
    def do_GET(self):
        ...
        self.is_request_blocked.wait()
        # request = self.mitm_request(request)
        if self.request:
            self._proxy_sock.sendall(self.request.to_data())
            #将响应信息返回给客户端
            ...
            self.send_error(404, 'self.request is None')
...
class MitmProxy(HTTPServer):
    ...
    def __init__(self,request_queue,response_queue,server_addr=('', 8788),RequestHandlerClass=ProxyHandle, bind_and_activate=True,https=True):
